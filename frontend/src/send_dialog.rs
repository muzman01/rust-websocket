...
pub fn send_dialog(props: &Props) -> Html{
    ...
    let on_message_change = Callback::from(move|e:Event|{
       ...
       if let Some(target) = target{
        cloned_new_message_handle.set(target.value());
       }
    });
    ...
    let on_username_apply_cancel_click = Callback::from(move |_: MouseEvent| {
    ...
    html! {
        <div class="input-group">
            ...
            <textarea onchange={on_message_change} value={new_message} class="form-control"></textarea>
            <button type="submit" onclick={on_button_click} class="btn-primary">{"Send"}</button>
        </div>
    }
}