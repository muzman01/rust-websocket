...
fn App() -> Html{
    ...
    let ws = use_websocket("ws://127.0.0.1:8000/chat".to_string());
    let mut cloned_messages= messages.clone();
    use_effect_with(ws.message.clone(), move | ws_message| {
       if let Some(ws_msg) = &**ws_message {
            let websocket_message:WebSocketMessage = serde_json::from_str::<common::WebSocketMessage>(&ws_msg).unwrap();
